...
def get_rgbd_stream():  # with depth and color aligned
    ...
    #     pcdata = [float(vtx[i][0]) * 1000, float(vtx[i][1]) * 1000, float(vtx[i][2]) * 1000,
    #               float(colorful[i][0]), float(colorful[i][1]), float(colorful[i][2])]
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # ud_thermal = undist(thermalImage, thermalIntrinsic, thermalDistortion, (480, 640))
        # cv2.imshow("1",ud_thermal)
        # cv2.waitKey(1)
        ...
        # fusion_img_JET = cv2.applyColorMap(fusion_img, cv2.COLORMAP_JET)


        real_fus_img = cv2.addWeighted(RGBImage_cut, 0.5, fusion_img_cut_JET, 0.5, 0)


        # real_fus_img = cv2.addWeighted(RGBImage, 0.5, fusion_img_JET, 0.5, 0)
